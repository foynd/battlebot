...
namespace UnityStandardAssets.Vehicles.Car
{
	...
	public class BotAIControl : MonoBehaviour
	{
		...
		void Update() {
			if (isActive) {
				GameObject.FindGameObjectWithTag("AI Saw").transform.Rotate (new Vector3 (0, 0, -2160) * Time.deltaTime);
			}
		}
		...
		[SerializeField] [Range(0, 1)] private float m_CautiousSpeedFactor = 0.075f;               // percentage of max speed to use when being maximally cautious
		...
		[SerializeField] private float m_ReachTargetThreshold = 0;                                // proximity to target to consider we 'reached' it, and stop driving.
		...
		private void OnCollisionStay(Collision col)
		{
			...
			{
				var otherAI = col.rigidbody.GetComponent<BotAIControl>();
				if (otherAI != null)
				{
